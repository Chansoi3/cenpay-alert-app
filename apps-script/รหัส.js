...
function doPost(e) {
  ...
  try {
    ...
    else if (action === "getDepositHistory") result = { status: "success", data: getDepositHistory(params.ref1) };
...
/**
 * ดึงประวัตินำฝากของชื่อบัญชี (Ref1) ในช่วง 1 เดือนย้อนหลัง
 * เทียบชื่อแบบ trim + lowercase (case-insensitive)
 * คืน: array ของ { date, StoreCode, StoreNameE, Amount, Txn, BillType, AM_Login, Type, Is_Abnormal, Check_Time, Remark }
 */
function getDepositHistory(ref1) {
  if (!ref1) return [];
  const target = ref1.toString().trim().toLowerCase();
  if (!target) return [];
  ...
  for (let i = 1; i < data.length; i++) {
    const rowRef1 = data[i][4] ? data[i][4].toString().trim().toLowerCase() : '';
    if (rowRef1 !== target) continue;

    const parsedDate = parseRowDate_(data[i][0]);
    if (!parsedDate || parsedDate < cutoff) continue;

    results.push({
      rowId: i + 1,
      date: formatDateString(data[i][0]),
      StoreCode: data[i][2],
      StoreNameE: data[i][3],
      Ref1: data[i][4],
      Amount: data[i][5],
  ...
  // เรียงใหม่สุดก่อน
  return results.sort((a, b) => {
    const da = parseRowDate_(a.date);
    const db = parseRowDate_(b.date);
    return db - da;
  });
}
